...
import {
  ...
  RefreshCw, Search, Plus, X, Check, Trash2, ChevronRight, ChevronDown,
  Clock, MapPin, AlertTriangle, CheckCircle2, Circle, Menu, ArrowRight,
  Pencil, Archive, Sparkles, ListChecks, Flag
} from "lucide-react";
...
const TOKENS = `
  :root{
    --paper:#EAEDEC; --panel:#F6F7F5; --panel-2:#FFFFFF;
    --ink:#1C2321; --ink-soft:#5B645F; --ink-faint:#8B928D;
    --line:#D7DBD6; --line-soft:#E3E6E1;
    --side-bg:#161F1C; --side-text:#C9D1CB; --side-dim:#77857F; --side-line:#26302C;
    --accent:#B8863B; --accent-ink:#4A3413; --accent-soft:#EFE1C8; --accent-soft-line:#DEC79A;
    --track:#3F7168; --track-soft:#DFEAE7;
    --risk:#B14D3D; --risk-soft:#F5E2DD;
    --blocked:#6B5B73; --blocked-soft:#E9E3EC;
    --overdue:#9A2F2F; --overdue-soft:#F5DEDB;
  }
...
function Modal({ title, onClose, children, width = "max-w-lg" }) {
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[8vh] px-4" style={{ background: "rgba(22,31,28,0.45)" }} onClick={onClose}>
        ...
        <SectionHeader action={
          <div className="flex gap-1">
            <input value={newDeliverable} onChange={e => setNewDeliverable(e.target.value)} placeholder="Add deliverable"
              ...
              className="text-xs px-2 py-1 rounded border border-[var(--line)] ppos-focus w-40" />
          </div>
    ...
    <div className="max-w-3xl mx-auto px-8 py-8">
      ...
      <div className="flex gap-2 mb-6">
        <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="New area name"
          ...
          className={inputCls} />
        <button onClick={() => { if (newName.trim()) { actions.addArea(newName.trim()); setNewName(""); } }} className={btnPrimary}>Add area</button>
      </div>

      <div className="space-y-2">
        {areas.map(a => {
          const count = projects.filter(p => p.areaId === a.id && !INACTIVE_PROJECT_STATUSES.has(p.status)).length;
          return (
            <div key={a.id} className="flex items-center justify-between p-3.5 rounded-lg border border-[var(--line)] bg-[var(--panel)]">
              <button onClick={() => openArea(a)} className="flex items-center gap-3 text-left flex-1">
                <span className="text-lg">{a.icon || "📁"}</span>
                <div>
                  <div className="font-ui text-sm text-[var(--ink)]">{a.name}</div>
                  <div className="text-xs font-ui text-[var(--ink-faint)]">{a.description}</div>
                </div>
              </button>
              <span className="font-mono text-xs text-[var(--ink-faint)] mr-3">{count} active</span>
              <button onClick={() => actions.archiveArea(a.id)} title="Archive" className="text-[var(--ink-faint)] hover:text-[var(--ink)] p-1"><Archive size={14} /></button>
            </div>
          );
        })}
      </div>
...
function BoardPage({ data, onOpenTask, actions }) {
  const cols = ["Inbox", "Next", "In Progress", "Waiting", "Done"];
        ...
        {cols.map((col, ci) => (
          <div key={col}>
            <div className="text-xs font-ui uppercase tracking-wide text-[var(--ink-faint)] mb-2 flex items-center justify-between">
              <span>{col}</span>
              <span className="font-mono">{active.filter(t => t.status === col).length}</span>
            </div>
            <div className="space-y-2 min-h-[60px]">
              {active.filter(t => t.status === col).map(t => (
                <div key={t.id} className="p-2.5 rounded-lg border border-[var(--line)] bg-[var(--panel)]">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                      ...
                      {ci < cols.length - 1 && <button onClick={() => move(t, 1)} className="text-[var(--ink-faint)] hover:text-[var(--ink)]"><ChevronRight size={13} /></button>}
                    </div>
                  </div>
                </div>
              ))}
...
function CalendarPage({ data, onOpenTask, onToggleDone }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-8 py-8">
      <h1 className="font-display text-3xl text-[var(--ink)] mb-6">Calendar</h1>
      <div className="space-y-5">
        {days.map(day => {
          const dayTasks = tasks.filter(t => t.status !== "Done" && t.status !== "Cancelled" && (t.scheduledDate === day || t.dueDate === day));
        ...
        {days.every(day => tasks.filter(t => t.status !== "Done" && t.status !== "Cancelled" && (t.scheduledDate === day || t.dueDate === day)).length === 0) && (
...
function Sidebar({ view, setView, areas, collapsed, setCollapsed }) {
  const nav = [
  ...
  return (
    <div className={classNames("shrink-0 h-full flex flex-col transition-all", collapsed ? "w-14" : "w-56")} style={{ background: "var(--side-bg)" }}>
        ...
        <>
          ...
          <nav className="px-2 space-y-0.5 flex-1 overflow-auto ppos-scroll">
            {areas.filter(a => a.status !== "Archived").map(a => (
              <button key={a.id} onClick={() => setView("areas")} className="w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded text-sm font-ui text-[var(--side-text)] hover:bg-[var(--side-line)]">
      ...
      <div className="px-2 pb-3 pt-2 border-t border-[var(--side-line)] space-y-0.5">
        <button onClick={() => setView("areas")} className={classNames("w-full flex items-center gap-2.5 px-2.5 py-2 rounded text-sm font-ui", view === "areas" ? "bg-[var(--side-line)] text-[var(--accent)]" : "text-[var(--side-text)] hover:bg-[var(--side-line)]")}>
...
function TopBar({ data, onQuickAdd, onWorkNow, onOpenProject, onOpenTask }) {
...
export default function App() {
  ...
  function createProject(fields) {
    ...
    setSelectedProjectId(created.id); setView("project-detail");
  }
  ...
  function openProject(p) { setSelectedProjectId(p.id); setView("project-detail"); }
  const selectedProject = data.projects.find(p => p.id === selectedProjectId);

  return (
    <div className="w-full h-full flex" style={{ background: "var(--paper)" }}>
      <style>{TOKENS}</style>
      <Sidebar view={view} setView={(v) => { setView(v); setSelectedProjectId(null); }} areas={data.areas} collapsed={collapsed} setCollapsed={setCollapsed} />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-auto ppos-scroll">
          {view === "today" && (
            <Dashboard data={data} openProject={openProject} openTask={setEditingTask}
              actions={{
                ...
                openNewProject: () => setShowNewProject(true), setView, toggleDone,
              }} />
          )}
          {view === "inbox" && (
          ...
          {view === "projects" && (
            <ProjectsPage data={data} openProject={openProject} actions={{ openNewProject: () => setShowNewProject(true) }} />
          )}
          ...
          {view === "areas" && (
            <AreasPage data={data} actions={{ addArea, archiveArea }} openArea={() => {}} />
          )}
          {view === "board" && (
      ...
      {showNewProject && (
        <NewProjectModal areas={data.areas} onClose={() => setShowNewProject(false)} onCreate={createProject} />
      )}
      {editingTask && (
